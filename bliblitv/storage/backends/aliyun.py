...
METADATA_PREFIX = 'x-oss-meta-'
DEFAULT_HOST = 'oss.aliyuncs.com'
PORT_BY_SECURITY = { True:443, False: 80 }

ACCESS_KEY = 'skgKAqCZZ6ig6bpR'
ACCESS_KEY_SECRET = 'RwkMAYQzg6ZUpq8WoYJNA14hmt1Ped'
...
def canonical_string(method, bucket="", key="", query_args={}, headers={}, expires=None):
    ...
    for header_key in headers:
        lk = header_key.lower()
        if lk in ['content-md5', 'content-type', 'date'] or lk.startswith(OSS_HEADER_PREFIX):
    ...
    if interesting_headers.has_key('x-oss-date'):
    ...
    for header_key in sorted_header_keys:
        if header_key.startswith(OSS_HEADER_PREFIX):
...
def encode(access_key_secret, str, urlencode=False):
    b64_hmac = base64.encodestring(hmac.new(access_key_secret, str, sha).digest()).strip()
    if urlencode:
...
class OSSAuthConnection:
    def __init__(self, access_key_id, access_key_secret,
            is_secure=False, server=DEFAULT_HOST, port=None, 
            calling_format=None):
        if not port:
            port = PORT_BY_SECURITY[is_secure]
    ...
    def put(self, bucket, key, object, headers={}):
        if not isinstance(object, OSSObject):
            object = OSSObject(object)
    ...
    def _add_oss_auth_header(self, headers, method,
            bucket, key, query_args):
        if not headers.has_key('Date'):
            headers['Date'] = get_gmtime()
        c_string = canonical_string(method, bucket, key, query_args, headers)
        headers['Authorization'] = \
            "OSS %s:%s" % (self.access_key_id, encode(self.access_key_secret, c_string))
#        headers['Authorization'] = get_authorization(self.access_key_id,self.access_key_secret, method, headers, resource=key)
#        print '@231',headers['Authorization']

    def _make_request(self, method, bucket='',
            key='', query_args={},headers={},
            data='', metadata={}):
        server = ''
        if bucket == '':
            server = self.server
        ...
        path += '/%s' % urllib.quote(key.encode('utf8'))
        ...
        while True:
            ...
            self._add_oss_auth_header(final_headers, method, bucket, key, query_args)

#            if (path.find('.gif')>=0 and len(data)>0):
#                import pdb
#                pdb.set_trace()

            connection.request(method, path, data, final_headers)
            resp = connection.getresponse()
#            print '@272','status==%d' % resp.status
...
class GetResponse(Response):
    def __init__(self, http_response):
        ...
        metadata = self.get_oss_metadata(response_headers)
        self.object = OSSObject(self.body, metadata)

    def get_oss_metadata(self, headers):
        metadata = {}
...
def get_test(url, header=None):
    if not header:
        header = {}
    r = requests.get(url, headers=header)

def put_object(url, headers=None, data=None,params=None):
    if not headers:
        headers = {}
    r = requests.put(url, headers=headers,data=data,params=params)
    return r

def get_expires(expire_after):
    t = time.time()
    return '%d' % (t + expire_after) 

def get_signature_in_url(access_key, access_key_secret, method, expires,resource=None,response_content_disposition=None):
    if not resource:
        resource = '/'
    if not response_content_disposition:
        response_content_disposition = ''
    str_to_sign = '%s\n\n\n%s\n%s?response-content-disposition=%s' % (method, expires, resource,response_content_disposition)
    h = hmac.new(access_key_secret,str_to_sign,sha)
    signature = base64.encodestring(h.digest()).strip()
    return urllib.quote(signature)

def get_signature(access_key, access_key_secret, method, header=None, resource=None):
    if not header:
        header = {}
    if not resource:
        resource = '/'
    ...
    canonicalized_oss_headers = get_canonicalized_oss_header(header) 
    canonicalized_resource = get_canonicalized_resource(resource)
    str_to_sign = '%s\n%s\n%s\n%s\n%s%s' % (method, content_md5.strip(),
            content_type, date, canonicalized_oss_headers,
            canonicalized_resource)
    h = hmac.new(access_key_secret, str_to_sign,sha)
    signature = base64.encodestring(h.digest()).strip()
    return signature

def get_authorization(access_key,access_key_secret, method, header=None, resource=None):
    signature = get_signature(access_key, access_key_secret,method,header,resource)
    result = 'OSS %s:%s' % (access_key,signature)
    return result

def get_canonicalized_oss_header(header):
    x_header = {}
    for k in header.keys():
        if isinstance(header[k],unicode):
            header[k] = header[k].encode('utf-8')

        if k.lower().startswith(OSS_HEADER_PREFIX):
            k_lower = k.lower()
            x_header[k_lower] = header[k]
        else:
            x_header[k] = header[k]
    ...
    if len(x_header)>0:
        ...
        x_header_keys.sort()
        for k in x_header_keys:
            if k.startswith(OSS_HEADER_PREFIX):
                canonicalized_oss_headers += k + ':' + x_header[k] + '\n'
    return canonicalized_oss_headers
   
def get_canonicalized_resource(resource):
    return resource 

def get_gmtime():
    str_gmtime= time.strftime('%a, %d %b %Y %X GMT',time.gmtime())
    return str_gmtime
...
def get_headers(bucket='',key='',secret='',method='',resource=None):
    if not resource:
        resource = '/'
    headers = {}
    if len(bucket) == 0:
        raise Exception('pls special')

def request_object(url,method,headers=None,params=None,data=None):
    if method == 'PUT':
        r = put_object(url, headers=headers, params=params, data=data) 

if __name__ == '__main__':
    bucket = 'hello-test'
    aliyun = ALIYUNConnection(ACCESS_KEY, ACCESS_KEY_SECRET,
            is_secure=False, server=DEFAULT_HOST, port=80, 
            calling_format=CallingFormat.SUBDOMAIN)
    r = aliyun.create_bucket('one-two-three')
    print 323,r.body
    r = aliyun.list_all_my_buckets()
    print 699,r.body
    r = aliyun.list_bucket('hello-test')
    print 702,r.body
    r = aliyun.delete_bucket('one-two-three')
    print 704,r.body
