...
try:
    from storages.backends.aliyun import OSSAuthConnection, QueryStringAuthGenerator, CallingFormat
except ImportError:
    raise ImproperlyConfigured("Could not load aliyun's oss bindings.\nSee "
        "http://help.aliyun.com/manual?spm=0.0.0.0.hXfQ5u&helpId=249")

ACCESS_KEY_NAME     = getattr(settings, 'OSS_S3_ACCESS_KEY_ID', getattr(settings, 'OSS_ACCESS_KEY_ID', None))
SECRET_KEY_NAME     = getattr(settings, 'OSS_S3_SECRET_ACCESS_KEY', getattr(settings, 'OSS_ACCESS_KEY_SECRET', None))
HEADERS             = getattr(settings, 'OSS_HEADERS', {})
DEFAULT_ACL         = getattr(settings, 'OSS_DEFAULT_ACL', 'public-read')
QUERYSTRING_ACTIVE  = getattr(settings, 'OSS_QUERYSTRING_ACTIVE', False)
QUERYSTRING_EXPIRE  = getattr(settings, 'OSS_QUERYSTRING_EXPIRE', 60)
SECURE_URLS         = getattr(settings, 'OSS_S3_SECURE_URLS', False)
BUCKET_PREFIX       = getattr(settings, 'OSS_BUCKET_PREFIX', '')
CALLING_FORMAT      = getattr(settings, 'OSS_CALLING_FORMAT', CallingFormat.SUBDOMAIN)
PRELOAD_METADATA    = getattr(settings, 'OSS_PRELOAD_METADATA', False)

IS_GZIPPED          = getattr(settings, 'OSS_IS_GZIPPED', False)
...
class OSSStorage(Storage):
    ...
    def __init__(self, bucket=settings.OSS_STORAGE_BUCKET_NAME,
        ...
        warnings.warn(
            "The oss backend is deprecated and will be removed in version 1.2. "
        ...
        self.connection = OSSAuthConnection(access_key, secret_key,
    ...
    def _get_connection(self):
        return OSSAuthConnection(*self._get_access_keys())
    ...
    def _put_file(self, name, content):
        ...
        if response.http_response.status not in (200, 206):
            raise IOError("OSSStorageError: %s" % response.message)
...
class PreloadingOSSStorage(OSSStorage):
    pass
